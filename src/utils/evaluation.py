...
from src.utils.chem import get_best_rmsd, set_rdmol_positions
from src.utils.geometry import to_zero_com
...
def get_pos_from_mol(mol, etkdg_version=3, mmff=False, mmff_iters=1000):
    if etkdg_version == 1:
...
class InferenceDataset4(Dataset):
    def __init__(self, data, n_samples, rc_mol_list, mmff=False, mmff_iters=1000):
        self.data = data
        self.n_samples = n_samples
        self.rc_mol_list = rc_mol_list
        self.mmff = mmff
    ...
    def __getitem__(self, idx):
        ...
        rdmol = data_.rdmol
        rc_conf = copy.deepcopy(self.rc_mol_list[idx])
        rdmol = set_rdmol_positions(rdmol, rc_conf)
        if self.mmff:
            rdmol = Chem.AddHs(rdmol, addCoords=True)
            AllChem.MMFFOptimizeMolecule(rdmol, mmffVariant='MMFF94s', maxIters=self.mmff_iters)
            rdmol = Chem.RemoveHs(rdmol)

        pos1 = torch.tensor(rdmol.GetConformer().GetPositions(), dtype=torch.float)
...
class InferenceDataset2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.etkdg_scale is not None:
            data_ = AddNoise(self.etkdg_scale)(data_)
...
class InferenceDataset(Dataset):
    def __init__(self, data, n_samples, etkdg_scale=None, etkdg_version=3, mmff=False, mmff_iters=1000):
        self.data = data
        self.n_samples = n_samples
        self.etkdg_scale = etkdg_scale
        self.etkdg_version = etkdg_version
        self.mmff = mmff
        self.mmff_iters = mmff_iters
...
def gen_samples(model, diffusion, data, positions, 
                device, bsz=32, etkdg_scale=None, etkdg_version=3,
                return_traj=False, num_workers=1,
                n_to_generate=None, from_torsion=False, 
                torsion_mol_list=None, rc_mol_list=None, dmcg_mol_list=None,
    ...
    if from_torsion:
        inference_dataset = InferenceDataset2(data, n_to_generate, torsion_mol_list, etkdg_scale, mmff, mmff_iters)
    elif rc_mol_list is not None:
        inference_dataset = InferenceDataset4(data, n_to_generate, rc_mol_list, mmff, mmff_iters)
    elif dmcg_mol_list is not None:
        inference_dataset = InferenceDataset4(data, n_to_generate, dmcg_mol_list, mmff, mmff_iters)
    else:
        inference_dataset = InferenceDataset(data, n_to_generate, etkdg_scale, etkdg_version, mmff, mmff_iters)
